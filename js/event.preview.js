...
* https://www.drupal.org/event/2815083
* @preserve
**/
...
(function ($, Drupal) {
  Drupal.behaviors.eventPreviewDestroyLinks = {
    attach: function attach(context) {
      function clickPreviewModal(event) {
        if (event.button === 0 && !event.altKey && !event.ctrlKey && !event.metaKey && !event.shiftKey) {
          ...
          var $previewDialog = $('<div>' + Drupal.theme('eventPreviewModal') + '</div>').appendTo('body');
      ...
      var $preview = $(context).find('.content').once('event-preview');
    ...
    detach: function detach(context, settings, trigger) {
      if (trigger === 'unload') {
        var $preview = $(context).find('.content').removeOnce('event-preview');
        if ($preview.length) {
