...
namespace Drupal\event\Plugin\Search;
...
/**
 * Handles searching for event entities using the Search module index.
 *
 * @SearchPlugin(
 *   id = "event_search",
 ...
 * )
 */
class EventSearch extends ConfigurableSearchPluginBase implements AccessibleInterface, SearchIndexingInterface {
  ...
  /**
   * The Renderer service to format the username and event.
   *
   ...
   *   matched against the {event_field_data} table.
  ...
  protected $advanced = [
    ...
    'author' => ['column' => 'n.uid'],
    'term' => ['column' => 'ti.tid', 'join' => ['table' => 'taxonomy_index', 'alias' => 'ti', 'condition' => 'n.eid = ti.eid']],
  ];
  ...
  /**
   * Constructs a \Drupal\event\Plugin\Search\EventSearch object.
   *
  ...
  public function isSearchExecutable() {
    // Event search is executable if we have keywords or an advanced parameter.
  ...
  protected function findResults() {
    ...
    $query->join('event_field_data', 'n', 'n.eid = i.sid AND n.langcode = i.langcode');
    ...
    $this->addEventRankings($query);

    // Run the query.
    $find = $query
      // Add the language code of the indexed item to the result of the query,
      // since the event will be rendered using the respective language.
      ->fields('i', ['langcode'])
  ...
  protected function prepareResults(StatementInterface $found) {
    ...
    $event_render = $this->entityManager->getViewBuilder('event');
    $keys = $this->keywords;

    foreach ($found as $item) {
      // Render the event.
      /** @var \Drupal\event\EventInterface $event */
      $event = $event_storage->load($item->sid)->getTranslation($item->langcode);
      $build = $event_render->view($event, 'search_result', $item->langcode);

      /** @var \Drupal\event\EventTypeInterface $type*/
      $type = $this->entityManager->getStorage('event_type')->load($event->bundle());
      ...
      $rendered .= ' ' . $this->moduleHandler->invoke('comment', 'event_update_index', [$event]);

      $extra = $this->moduleHandler->invokeAll('event_search_result', [$event]);

      $language = $this->languageManager->getLanguage($item->langcode);
      $username = [
        '#theme' => 'username',
        '#account' => $event->getOwner(),
      ];

      $result = [
        'link' => $event->url('canonical', ['absolute' => TRUE, 'language' => $language]),
        'type' => $type->label(),
        'title' => $event->label(),
        'event' => $event,
        'extra' => $extra,
      ...
      // We have to separately add the event owner's cache tags because search
      ...
      // explicitly here.
      $this->addCacheableDependency($event->getOwner());
   ...
   *
   * This information is being removed from the rendered event that is used to
  ...
  public function updateIndex() {
    // Interpret the cron limit setting as the maximum number of events to index
    // per cron run.
    $limit = (int) $this->searchSettings->get('index.cron_limit');

    $query = db_select('event', 'n', ['target' => 'replica']);
    $query->addField('n', 'eid');
    $query->leftJoin('search_dataset', 'sd', 'sd.sid = n.eid AND sd.type = :type', [':type' => $this->getPluginId()]);
    ...
    $event_storage = $this->entityManager->getStorage('event');
    foreach ($event_storage->loadMultiple($eids) as $event) {
      $this->indexEvent($event);
    }
  }

  /**
   * Indexes a single event.
   *
   * @param \Drupal\event\EventInterface $event
   *   The event to index.
   */
  protected function indexEvent(EventInterface $event) {
    $languages = $event->getTranslationLanguages();
    $event_render = $this->entityManager->getViewBuilder('event');

    foreach ($languages as $language) {
      $event = $event->getTranslation($language->getId());
      // Render the event.
      $build = $event_render->view($event, 'search_index', $language->getId());
      ...
      $extra = $this->moduleHandler->invokeAll('event_update_index', [$event]);
      foreach ($extra as $t) {
        $text .= $t;
      }

      // Update index, using search index "type" equal to the plugin ID.
      search_index($this->getPluginId(), $event->id(), $language->getId(), $text);
    }
  }
  ...
  public function indexClear() {
    // All EventSearch pages share a common search index "type" equal to
    // the plugin ID.
  ...
  public function indexStatus() {
    $total = $this->database->query('SELECT COUNT(*) FROM {event}')->fetchField();
    $remaining = $this->database->query("SELECT COUNT(DISTINCT n.eid) FROM {event} n LEFT JOIN {search_dataset} sd ON sd.sid = n.eid AND sd.type = :type WHERE sd.sid IS NULL OR sd.reindex <> 0", [':type' => $this->getPluginId()])->fetchField();
  ...
  public function searchFormAlter(array &$form, FormStateInterface $form_state) {
    ...
    // Add event types.
    $types = array_map(['\Drupal\Component\Utility\Html', 'escape'], event_type_get_names());
