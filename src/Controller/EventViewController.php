...
/**
 * Defines a controller to render a single event.
 */
class EventViewController extends EntityViewController {
  ...
  /**
   * Creates an EventViewController object.
   *
   ...
   * {@inheritdoc}
   */
  public function view(EntityInterface $event, $view_mode = 'full', $langcode = NULL) {
    $build = parent::view($event, $view_mode, $langcode);

    foreach ($event->uriRelationships() as $rel) {
      $url = $event->toUrl($rel);
      ...
      if ($this->currentUser->isAuthenticated() || $url->access($this->currentUser)) {
        // Set the event path as the canonical URL to prevent duplicate content.
  ...
  /**
   * The _title_callback for the page that renders a single event.
   *
   ...
   *   The page title.
   */
  public function title(EntityInterface $event) {
    return $this->entityManager->getTranslationFromContext($event)->label();
  }
